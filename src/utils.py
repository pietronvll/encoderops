...
def _DISULFIDE_DIHEDRALS(traj, sincos=True):
    ...
    for i, j in itertools.combinations(s_cys, 2):
        ...
        if d_ij < 0.25:
            ...
            for k, bonded_atom, _, _ in bonds:
                if int(k) == i:
                    c_i = int(bonded_atom)
                elif int(bonded_atom) == i:
                    c_i = int(k)
                if int(k) == j:
                    c_j = int(bonded_atom)
                elif int(bonded_atom) == j:
                    c_j = int(k)
...
def get_residuals(eigvals, Q, phi_X, phi_Y):
...
class EpochTimerCallback(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        if self.epoch_start_time is not None:
            ...
            pl_module.log('epoch_time', epoch_time, on_epoch=True, prog_bar=False)
